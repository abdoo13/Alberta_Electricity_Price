...
if st.sidebar.button("Predict Electricity Prices"):
  ...
  for col in list(df_3.columns)[2:]:
    df_3[col] = df[col][0]
  df_3['Electricity Price (CAD Cents/KWh)'] = model.predict(df_3.values) #cols_3[-1]
  #st.dataframe(df_3)
  ...
  c = (alt.Chart(df_3).mark_line().encode(
                                        x = alt.X(cols_3[0]),
                                        y = alt.Y('Electricity Price (CAD Cents/KWh)', scale=alt.Scale(domain=[np.round(df_3['Electricity Price (CAD Cents/KWh)'].min(), 1)-0.2, 
